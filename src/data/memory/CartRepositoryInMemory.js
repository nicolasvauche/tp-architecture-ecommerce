  ...
  return {
    async get() {
      const items = db.cart_items.map((ci) => {
        ...
        return { productId: ci.product_id, quantity: ci.qty, price };
      });
    ...
    async addItem({ productId, quantity }) {
      ...
      const existing = db.cart_items.find((ci) => ci.product_id === pid);
      if (existing) {
        existing.qty += quantity;
      } else {
        db.cart_items.push({ product_id: pid, qty: quantity });
      }
    },

    async removeItem(productId) {
      const pid = Number(productId);
      db.cart_items = db.cart_items.filter((ci) => ci.product_id !== pid);
    },
